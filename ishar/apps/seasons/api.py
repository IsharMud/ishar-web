...
@api.get(path="/season/", response=SeasonSchema, tags=["seasons"])
def current(request):
    """Current season."""
    return Season.objects.get(season_id=get_current_season().season_id)


@api.get(path="/season/{season_id}/", response=SeasonSchema, tags=["seasons"])
def season(request, season_id: int):
    """Single season, by ID."""
    return get_object_or_404(Season, season_id=season_id)


@api.get(path="/seasons/", response=List[SeasonSchema], tags=["seasons"])
def seasons(request):
    """All seasons."""
