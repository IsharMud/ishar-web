...
class PlayerSkills(models.Model):
    skill = models.OneToOneField('SpellInfo', models.DO_NOTHING, primary_key=True)  # The composite primary key (skill_id, player_id) found, that is not supported. The first column is selected.
    player = models.ForeignKey('Players', models.DO_NOTHING)
...
class QuestPrereqs(models.Model):
    quest = models.OneToOneField('Quests', models.DO_NOTHING, primary_key=True)  # The composite primary key (quest_id, required_quest) found, that is not supported. The first column is selected.
...
class SpellInfoBck(models.Model):
